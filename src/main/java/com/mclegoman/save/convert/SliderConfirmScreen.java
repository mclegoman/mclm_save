...
public class SliderConfirmScreen extends Screen {
	...
	protected final void buttonClicked(ButtonWidget buttonWidget) {
		if (buttonWidget.active) {
			if (buttonWidget.id == 0) {
				...
				this.value = ((SliderWidget) buttonWidget).onPressed(this.minecraft, this.width);
	...
	public final void render(int i, int j, float f) {
		...
		SliderWidget slider = (SliderWidget)buttons.get(0);
		if (slider.focused) {
			if (Mouse.isButtonDown(0)) this.value = slider.onPressed(this.minecraft, this.width);
			else slider.focused = false;
		}
		super.render(i, j, f);
	}
}
